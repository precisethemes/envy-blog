 ...
 */
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Envy_Blog_Breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  1.0.0
 * @access public
 * @param  array $args Arguments to pass to Envy_Blog_Breadcrumb.
 * @return void
 */
function envy_blog_breadcrumb( $args = array() ) {

    $breadcrumb = apply_filters( 'envy_blog_breadcrumb_object', null, $args );

    if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Envy_Blog_Breadcrumb( $args );
...
class Envy_Blog_Breadcrumb {
    ...
    /**
     * Sets up the breadcrumb trail properties.  Calls the `Envy_Blog_Breadcrumb::add_items()` method
     ...
     *     @type array     $labels         Text labels. @see Envy_Blog_Breadcrumb::set_labels()
     *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Envy_Blog_Breadcrumb::set_post_taxonomy()
    ...
    public function __construct( $args = array() ) {
        ...
        $this->args = apply_filters( 'envy_blog_breadcrumb_args', wp_parse_args( $args, $defaults ) );
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            $breadcrumb = sprintf(
                '<%1$s role="navigation" aria-label="%2$s" class="envy-blog-breadcrumbs breadcrumbs">%3$s%4$s%5$s</%1$s>',
        ...
        $breadcrumb = apply_filters( 'envy_blog_breadcrumb', $breadcrumb, $this->args );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            ...
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'envy-blog' ),
        ...
        $this->labels = apply_filters( 'envy_blog_breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
    }
    ...
    protected function set_post_taxonomy() {
        ...
        $this->post_taxonomy = apply_filters( 'envy_blog_breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
    }
    ...
    protected function add_items() {
        ...
        $this->items = array_unique( apply_filters( 'envy_blog_breadcrumb_items', $this->items, $this->args ) );
    }
